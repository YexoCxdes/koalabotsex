...
async def verify(ctx):
    ...
    role = get(ctx.message.author.guild.roles, name='Verified')
    msg = await ctx.send(f'React to the message to verify <@{person.id}>')
    await msg.add_reaction('✅')
    reaction = await koala.wait_for('reaction_add', check=check)
